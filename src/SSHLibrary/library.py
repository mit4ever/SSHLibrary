...
class SSHLibrary(object):
    ...
    # =============================================
    # === MỚI: Open SFTP-only Connection ===
    # =============================================
    def open_sftp_smart_connection(self, host, alias=None, port=22, username=None, password=None,
                             keyfile=None, timeout=None, encoding=None, encoding_errors=None):
        """
        Opens an SFTP-only connection (no shell).
        For servers: "This service allows sftp connections only."
        """
        ...
        try:
            ...
            if keyfile:
                ...
                if not username or not password:
                    raise ValueError("username and password required for SFTP")
                transport.auth_password(username, password)

            sftp = SFTPClient.from_transport(transport)

            # === SFTPOnlyClient: KHÔNG KẾ THỪA AbstractSSHClient ===
            class SFTPOnlyClient:
                def __init__(self, sftp, transport, host, port, alias):
                    self.sftp = sftp
                ...
                def get_file(self, source, destination='.', scp='OFF', scp_preserve_times=False):
                    ...
                    if '*' in source or '?' in source or '[' in source:
                        ...
                        files = [f for f in self.sftp.listdir(dir_path) if fnmatch(f, pattern)]
                        transferred = []
                        for f in files:
                            src = os.path.join(dir_path, f)
                            dst = destination if os.path.isdir(destination) else os.path.dirname(destination)
                ...
                # === STAT (cho Get File Stat) ===
                def stat(self, path):
                    return self.sftp.stat(path)

# === THÊM TỪ ĐÂY TRỞ XUỐNG (thụt lề 16 space hoặc 4 tab) ===
                def list_files_in_dir(self, path='.', pattern=None, absolute=False):
                    from fnmatch import fnmatch
                    from stat import S_ISDIR
                    try:
                        items = self.sftp.listdir_attr(path)
                        files = [item.filename for item in items if not S_ISDIR(item.st_mode)]
                        if pattern:
                ...
                def list_dirs_in_dir(self, path='.', pattern=None, absolute=False):
                    ...
                    try:
                        items = self.sftp.listdir_attr(path)
                        dirs = [item.filename for item in items if S_ISDIR(item.st_mode)]
                        if pattern:
                        ...
                        raise RuntimeError(f"Failed to list dirs in '{path}': {e}")
                ...
                def list_dir(self, path='.', pattern=None):
                    ...
                    try:
                        ...
                        if pattern:
                            items = [i for i in items if fnmatch(i, pattern)]
                        return items
                    except Exception as e:
                        raise RuntimeError(f"Failed to list directory '{path}': {e}")
            # === TẠO CLIENT ===
            client = SFTPOnlyClient(sftp, transport, host, port, alias)
            index = self._connections.register(client, alias)
            client.config.index = index
            self._log(f"SFTP-only connection opened: {host}:{port}", 'INFO')
            return index
    ...
    def get_file_stat(self, path):
        """
        Returns file/directory metadata via SFTP.
        """
        try:
            from stat import S_ISDIR  # ← Import stat để kiểm tra mode
            attr = self.current.stat(path)  # ← attr là SFTPAttributes

            return {
                'size': attr.st_size,
                'mtime': attr.st_mtime,
                'atime': attr.st_atime,
                'permissions': oct(attr.st_mode),  # e.g., '0o100644'
                'uid': attr.st_uid,
                'gid': attr.st_gid,
                'is_dir': S_ISDIR(attr.st_mode)   # ← DÙNG S_ISDIR thay vì .isdir()
            }
        except Exception as e:
            raise RuntimeError(f"Failed to stat '{path}': {e}")
        
    # =============================================
    # === MỚI: Get File Modification Time ===
    # =============================================
    
    def get_file_modification_time(self, path, format_date="%Y-%m-%d %H:%M:%S"):
        """
        Trả về thời gian sửa đổi dưới dạng dấu thời Unix timestamp.

        Example:
        | ${mtime}=    Get File Modification Time    /var/log/auth.log  format_date=%Y-%m-%d %H:%M:%S | 
        | ${date}=     Convert Time                  ${mtime}    %Y-%m-%d %H:%M |
        |
        """
        try:
            mtime = self.current.sftp.stat(path).st_mtime
            return datetime.fromtimestamp(mtime).strftime(format_date)
        except Exception as e:
            raise RuntimeError(f"Failed to get mtime for '{path}': {e}")

    # =============================================
    # === Các keyword cũ giữ nguyên ===
    # =============================================
